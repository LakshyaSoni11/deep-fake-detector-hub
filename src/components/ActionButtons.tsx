...
const ActionButtons = () => {
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 bg-gradient-to-b from-softblue/30 to-background/0 z-0"></div>
...
const ActionButton = ({ title, description, url, type }: ActionButtonProps) => {
  return (
    ...
    >
      <div className={`w-16 h-16 rounded-full flex items-center justify-center mb-4 transition-colors duration-300 ${type === 'text' ? 'bg-deepblue/10 group-hover:bg-deepblue/20' : 'bg-coral/10 group-hover:bg-coral/20'}`}>
        {type === 'text' ? (
          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={`text-deepblue`}>
      ...
      <h3 className={`text-xl font-display font-semibold mb-2 ${type === 'text' ? 'text-deepblue' : 'text-coral'}`}>{title}</h3>
      
      <p className="text-charcoal/70 mb-4">{description}</p>
      
      <span className={`inline-flex items-center font-medium ${type === 'text' ? 'text-deepblue' : 'text-coral'} group-hover:underline`}>
        Try Now
